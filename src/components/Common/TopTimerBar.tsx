...
export function TopTimerBar() {
  ...
  return (
    ...
    <div className={`fixed top-4 right-8 z-50 flex items-center gap-4 border-[3px] rounded-full px-7 py-3 ${progress * 100 <= 30 ? 'border-red-500' : backgroundVariant === 'image-white' ? 'border-[#303030]' : 'border-[#FCF8EF]'}`}>
      <img src={progress * 100 <= 30 ? iconTimerRed : backgroundVariant === 'image-white' ? iconTimerBlack : iconTimerWhite} alt="clock" className="w-8 h-8" />
      <span className={`font-gaming text-2xl tracking-widest ${progress * 100 <= 30 ? 'text-red-500' : backgroundVariant === 'image-white' ? 'text-[#303030]' : 'text-[#FCF8EF]'}`} style={{ fontVariantNumeric: 'tabular-nums' }}>
